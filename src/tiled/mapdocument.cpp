    ...
    mUndoStack(new QUndoStack(this))
{
    ...
    connect(mMapObjectModel, SIGNAL(rowsInserted(QModelIndex,int,int)),
            SLOT(onObjectsInserted(QModelIndex,int,int)));
    connect(mMapObjectModel, SIGNAL(rowsRemoved(QModelIndex,int,int)),
            SLOT(onObjectsInsertedOrRemoved(QModelIndex,int,int)));
...
void MapDocument::onObjectsInserted(const QModelIndex &parent, int first, int last)
{
    emit objectsInserted(mMapObjectModel->toObjectGroup(parent), first, last);
    onObjectsInsertedOrRemoved(parent, first, last);
}

void MapDocument::onObjectsInsertedOrRemoved(const QModelIndex &parent, int first, int last)
{
    Q_UNUSED(first)
