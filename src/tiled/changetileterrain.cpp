...
{
    QMap<Tile *, Change>::const_iterator i = mChanges.constBegin();
...
bool ChangeTileTerrain::mergeWith(const QUndoCommand *other)
{
    ...
    QMap<Tile *, Change>::const_iterator i = o->mChanges.constBegin();
    QMap<Tile *, Change>::const_iterator i_end = o->mChanges.constEnd();
    while (i != i_end) {
        Tile *tile = i.key();
        const Change &change = i.value();
